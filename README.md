...
* **Password strength** — new passwords (account creation and password change) must be at least 8 characters and contain both a letter and a number, enforced via a Pydantic validator shared by both schemas
...
* **No automated dependency vulnerability scanning** — Dependabot alerts/security updates aren't enabled on this repo. For a showcase project (not a live production service handling real user data), the tradeoff is reasonable; this would be one of the first things enabled before any real deployment.
